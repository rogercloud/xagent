...
class AgentToolResult(BaseModel):
    """Declared return shape of ``AgentTool``.

    ``response``/``file_outputs`` are the happy-path fields and the only ones
    this model is ever constructed with. The classification fields document
    the classified-failure envelope: those returns are hand-built dicts
    (``_classified_failure`` is the constructor of record), never instances
    of this model, but ``return_type()`` is the tool's declared output
    contract — a consumer that model-validates and re-dumps a tool result
    must not be able to strip the classification keys and leave a bare
    ``response`` that ``tool_result_succeeded`` reads as a success. They stay
    ``None`` on the happy path and are dropped by
    ...
    success: Optional[bool] = Field(
        default=None, description="Whether the delegated execution succeeded"
    )
    is_error: Optional[bool] = Field(
        default=None, description="Whether the result represents an error"
    )
    status: Optional[str] = Field(
        default=None, description="Delegated execution status classification"
    )
    failure_code: Optional[str] = Field(
        default=None, description="Allowlisted public tool failure classification"
    )
    error: Optional[str] = Field(
        default=None, description="Public-safe failure message when available"
    )
    output: Optional[str] = Field(
        default=None, description="Model-facing text when execution did not succeed"
    )
...
def _classify_delegated_child_failure(
    ...
    """Classify a delegated child result that must not reach the parent as a success.

    Runs before the child's ``output`` is read so a paused or failed child
    body never reaches the parent disguised as a completed answer. Returns
...
class AgentTool(AbstractBaseTool):
    ...
    async def run_json_async(self, args: Mapping[str, Any]) -> Any:
        ...
        try:
            ...
            # A child that paused for user input or failed outright must be
            # classified before ``output`` is ever read — otherwise a waiting
            # child's partial answer, or a failed child's error text, would
            # be read as ``output`` and returned to the parent as a normal
            # completed response.
