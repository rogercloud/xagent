...
const addTextAuto = (slide, text, options = {{}}, role = 'body') => {{
  slide.addText(text, getTextOptions(text, options, role));
}};
...
const addNumberedBullet = (slide, num, text, x, y) => {{
  ...
  addTextAuto(slide, num.toString(), {{
    x: x, y: y, w: 0.4, h: 0.5,
...
"""
    ...
    def _build_title_slide_js(
        ...
        js_lines = [
            ...
            f"addTextAuto(_slide, {json.dumps(title, ensure_ascii=False)}, {{ x: 0.5, y: 1.5, w: 9, h: 1.5, fontSize: titleSize, bold: true, color: '{text_color}', align: 'center', valign: 'middle' }}, 'title');",
        ]

        if subtitle:
            js_lines.append(
                f"addTextAuto(_slide, {json.dumps(subtitle, ensure_ascii=False)}, {{ x: 0.5, y: 3.2, w: 9, h: 0.8, fontSize: {body_size + 2}, color: '{accent_color}', align: 'center' }});"
            )
    ...
    def _build_quote_slide_js(
        ...
        js_lines.append(
            f"addTextAuto(_slide, {json.dumps(text, ensure_ascii=False)}, {{ x: 1, y: 2.5, w: 8, h: 2, fontSize: quoteSize, bold: true, color: {json.dumps(colors.get('text', '#1E2761'), ensure_ascii=False)}, align: 'center' }}, 'title');"
        )
        js_lines.append("")
        return "\n".join(js_lines) + "\n"
    ...
    def _build_thank_you_slide_js(
        ...
        js_lines.append(
            f"addTextAuto(_slide, {json.dumps(text, ensure_ascii=False)}, {{ x: 1, y: 2.5, w: 8, h: 2, fontSize: tySize, bold: true, color: {json.dumps(colors.get('text', '#1E2761'), ensure_ascii=False)}, align: 'center' }}, 'title');"
        )
        js_lines.append("")
        return "\n".join(js_lines) + "\n"
    ...
    def _build_metrics_slide_js(
        ...
        if title:
            js_lines.append(
                f"addTextAuto(_slide_metrics, {json.dumps(title, ensure_ascii=False)}, {{ x: 0.5, y: 0.5, w: 9, h: 0.5, fontSize: 18, bold: true, color: {json.dumps(text_color, ensure_ascii=False)} }}, 'title');"
            )
        ...
        for i, item in enumerate(items):
            ...
            js_lines.append(
                f"addTextAuto(_slide_metrics, {json.dumps(value, ensure_ascii=False)}, {{ x: {x_pos}, y: {y_val}, w: 4, h: 1.5, fontSize: 32, bold: true, color: {json.dumps(text_color, ensure_ascii=False)}, align: 'center' }}, 'title');"
            )
            ...
            js_lines.append(
                f"addTextAuto(_slide_metrics, {json.dumps(label, ensure_ascii=False)}, {{ x: {x_pos}, y: {y_label}, w: 4, h: 0.5, fontSize: metric{i}LabelSize, color: {json.dumps(text_color, ensure_ascii=False)}, align: 'center' }});"
            )
    ...
    def _build_timeline_slide_js(
        ...
        for i, ms in enumerate(milestones):
            ...
            js_lines.append(
                f"addTextAuto(_slide_timeline, {json.dumps(ms_title, ensure_ascii=False)}, {{ x: {x_pos}, y: 1.5, w: 2, h: 0.5, fontSize: ms{i}TitleSize, bold: true, color: {json.dumps(text_color, ensure_ascii=False)} }}, 'title');"
            )
            ...
            js_lines.append(
                f"addTextAuto(_slide_timeline, {json.dumps(ms_desc, ensure_ascii=False)}, {{ x: {x_pos}, y: 2.1, w: 2, h: 2.5, fontSize: ms{i}DescSize, color: {json.dumps(text_color, ensure_ascii=False)} }});"
            )
    ...
    def _build_comparison_slide_js(
        ...
        js_lines.append(
            f"addTextAuto(_slide_comparison, {json.dumps(left_title, ensure_ascii=False)}, {{ x: 0.5, y: 1, w: 4, h: 0.5, fontSize: leftTitleSize, bold: true, color: {json.dumps(text_color, ensure_ascii=False)} }}, 'title');"
        )
        js_lines.append(
            f"addTextAuto(_slide_comparison, {json.dumps(right_title, ensure_ascii=False)}, {{ x: 5.5, y: 1, w: 4, h: 0.5, fontSize: rightTitleSize, bold: true, color: {json.dumps(text_color, ensure_ascii=False)} }}, 'title');"
        )
        js_lines.append(
            f"addTextAuto(_slide_comparison, 'VS', {{ x: 4.6, y: 2, w: 0.8, h: 0.5, fontSize: 14, bold: true, color: {json.dumps(colors.get('accent', '#FFFFFF'), ensure_ascii=False)}, align: 'center' }}, 'title');"
        )
        js_lines.append("")
        ...
        for i, item in enumerate(left_items):
            ...
            js_lines.append(
                f"addTextAuto(_slide_comparison, {json.dumps(item, ensure_ascii=False)}, {{ x: 0.5, y: {y_pos}, w: 4, h: 0.5, fontSize: leftItem{i}Size, color: {json.dumps(text_color, ensure_ascii=False)} }});"
            )
        ...
        for i, item in enumerate(right_items):
            ...
            js_lines.append(
                f"addTextAuto(_slide_comparison, {json.dumps(item, ensure_ascii=False)}, {{ x: 5.5, y: {y_pos}, w: 4, h: 0.5, fontSize: rightItem{i}Size, color: {json.dumps(text_color, ensure_ascii=False)} }});"
            )
...
addTextAuto(_slide_statement, {json.dumps(text, ensure_ascii=False)}, {{
  x: 1,
  y: 2.5,
  w: 8,
  h: 2,
...
addTextAuto(_slide_image, {json.dumps(title, ensure_ascii=False)}, {{
  x: 0.5,
  y: 0.5,
  w: 9,
  h: 0.5,
...
addTextAuto(_slide_image, {json.dumps(caption, ensure_ascii=False)}, {{
  x: 0.5,
  y: 5.5,
  w: 9,
  h: 0.5,
...
"""
    ...
    def _build_flow_slide_js(
        ...
        for i, step in enumerate(steps):
            ...
            js_lines.append(
                f"addTextAuto(_slide_flow, {json.dumps(step, ensure_ascii=False)}, {{ x: {x}, y: 2.5, w: 2, h: 1, fontSize: flowStep{i}Size, color: {json.dumps(text_color, ensure_ascii=False)}, align: 'center', valign: 'middle' }});"
            )
