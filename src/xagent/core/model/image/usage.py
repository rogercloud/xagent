...
from ...tools.core.media_usage import is_usable_model_name
...
def _billing_identity(value: Any) -> str:
    """A model identity fit to bill, or "" when the value is a placeholder.

    Shares the placeholder set with the rest of media billing rather than
    restating it: "", "none", "null" and "default" are configuration standing in
    for a model, not models. The aggregator groups on `model_id or model`, so
    letting one through would open a billing line named "default" that unrelated
    models merge into.

    Returns "" rather than a fallback string, because an empty field is what the
    row already means by "not known" -- and with both fields filtered, a row that
    resolves to neither still records (the call happened) under an empty
    identity, which the saas side reports as unidentified rather than billing.
    """
    return value if is_usable_model_name(value) else ""
...
def _is_authoritative_zero(value: Any) -> bool:
    ...
    The distinction the alias scan needs: a payload stating ``0`` has answered,
    so the scan stops; a value that merely truncates to zero (``-0.5``,
    ``1e-9``) has not, and a later alias may hold the real count.

    Numeric types answer that by comparing to ``0``. Text does not: ``"0" == 0``
    is False, so an explicit string zero read as "not the provider's answer" and
    the scan moved on to a later alias -- the one case where the boundary and
    this check disagreed, since the boundary bills ``"0"`` as 0 like any other
    numeric string. Text is therefore compared after the same conversion the
    boundary applies.
    """
    if isinstance(value, (str, bytes, bytearray)):
        try:
...
def _countable(value: Any) -> bool:
    ...
    * absurd magnitudes -- bounded like a reported count, so a 401-digit
    ...
    try:
        ...
        as_int = int(value)
        if as_int == 0:
            return _is_authoritative_zero(value)
...
def _usable_image_count(value: Any) -> Optional[int]:
...
def _first_usable_count(payload: Any, *names: str) -> Optional[int]:
    ...
    for name in names:
        try:
            reported = _usable_image_count(payload.get(name))
        except Exception as e:  # noqa: BLE001
...
def _reported_dimensions(payload: Any) -> str:
    ...
    for width_key, height_key in (
        ...
        try:
            width = _usable_image_count(payload.get(width_key))
            height = _usable_image_count(payload.get(height_key))
        except Exception as e:  # noqa: BLE001
...
def record_unusable_response(
    *,
    ...
    """Record a billed call whose response never became readable.

    The provider answered 200 and charged for it, but the body could not be
    decoded or the SDK raised instead of returning one, so there is no usage
    payload: the row carries the requested count and no tokens. That is what a
    zero-token row already means in media accounting -- the call happened, was
    charged, and its size is unmeasured.

    Recording is the half that is easy to forget. Classifying the failure as
    non-retryable stops the repeated charges, but on its own it leaves the one
    real charge invisible, which is the worse of the two failures: the money is
    gone either way, and without the row nothing shows it. Every provider that
    can fail after the charge calls this on the way out.
    """
    record_image_usage(
        {},
    ...
    *,
    ...
    try:
        ...
        add_media_usage(
            call_type=call_type,
            quantity=quantity,
            # Filtered through the shared placeholder rule rather than passed
            # raw. The aggregator groups on `model_id or model`, so a configured
            # id of "" / "none" / "null" / "default" would itself become a
            # billing identity, merging unrelated models onto one invoice line.
            # Both fields are filtered because either can become that key.
            model=_billing_identity(model_name),
            model_id=_billing_identity(model_id),
        ...
        # fact, so a metering bug has to be diagnosable from the log alone
        # -- same reason media_usage.record_media_usage does it.
        logger.warning("Failed to record image usage: %s", e, exc_info=True)
