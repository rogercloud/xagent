...
class SlackBotInstance:
    ...
    async def handle_events_api_payload(self, payload: dict[str, Any]) -> None:
        ...
        durable_input = get_shared_task_execution_enabled() and self._message_text(
            event
        ).casefold() not in (_CONTROL_START_COMMANDS | _CONTROL_NEW_COMMANDS)
        if not durable_input and dedup_key and not self._remember_event_id(dedup_key):
            return
