...
@pytest.mark.parametrize(
    "queued_kind",
    [
        TaskCommandKind.START,
        TaskCommandKind.RESUME_INPUT,
        TaskCommandKind.MESSAGE,
        TaskCommandKind.RESUME,
    ],
)
async def test_worker_capacity_keeps_new_execution_pending_and_controls_available(
    db_session, monkeypatch, queued_kind
):
    from xagent.web.services.task_coordinator_runtime import TaskCoordinatorRegistry
    from xagent.web.services.task_worker_capacity import TaskWorkerCapacity

    monkeypatch.setenv("XAGENT_SHARED_TASK_EXECUTION_ENABLED", "true")
    monkeypatch.setenv("XAGENT_TASK_WORKER_MAX_CONCURRENT_TASKS", "1")
    monkeypatch.setattr(
        task_command_transport_module, "worker_capacity", TaskWorkerCapacity()
    )
    user, first = _create_running_task(db_session)
    first.runner_id = None
    ...
    start = enqueue(first, TaskCommandKind.START, "capacity-start")
    waiting = enqueue(second, queued_kind, "capacity-waiting")
    registry = TaskCoordinatorRegistry()
    ...
    async def execute(command):
        ...
        if command.kind == TaskCommandKind.START and command.task_id == first.id:
            ...
            async def schedule():
                ...
                task_execution_service.background_task_manager.register_task(
                    command.task_id, handle
                )

            await coordinator.execute_command(command, schedule)
        return {}

    try:
        assert await dispatch_one_task_command(execute, command_db_id=start.command_id)
        # The START command is complete, but its Agent still owns the only slot.
        assert not await dispatch_one_task_command(
            execute, command_db_id=waiting.command_id
        )
        await dispatch_task_command_promptly(execute, command_db_id=waiting.command_id)
        for kind in (
            TaskCommandKind.MESSAGE,
            TaskCommandKind.RESUME,
            ...
            control = enqueue(first, kind, "capacity-" + kind.value)
            # Polling must skip the older, capacity-blocked command for another task.
            assert await dispatch_one_task_command(execute)
            db_session.expire_all()
            assert (
                db_session.get(TaskExecutionCommand, control.command_id).status
                == COMMAND_COMPLETED
            )
        db_session.expire_all()
        stored = db_session.get(TaskExecutionCommand, waiting.command_id)
        assert (stored.status, stored.attempt_count, stored.defer_count) == (
            COMMAND_PENDING,
            0,
            0,
        )
        if queued_kind == TaskCommandKind.START:
            handles[0].cancel()
        else:
            unblock.set()
        await asyncio.wait_for(finalizing.wait(), 5)
        # Keep the slot until finalizers finish, including after cancellation.
        assert not await dispatch_one_task_command(
            execute, command_db_id=waiting.command_id
        )
        cleanup.set()
...
async def test_worker_capacity_releases_unscheduled_command(
    ...
    monkeypatch.setenv("XAGENT_SHARED_TASK_EXECUTION_ENABLED", "true")
    monkeypatch.setenv("XAGENT_TASK_WORKER_MAX_CONCURRENT_TASKS", "1")
    capacity = TaskWorkerCapacity()
    monkeypatch.setattr(task_command_transport_module, "worker_capacity", capacity)
    user, task = _create_running_task(db_session)
    task.runner_id = None
    task.lease_expires_at = None
    db_session.commit()
    ...
    dispatch = asyncio.create_task(
        dispatch_one_task_command(execute, command_db_id=command.command_id)
    )
    ...
    assert capacity.admitted_tasks_when_full() is None


def test_worker_capacity_reserves_atomically_across_claim_threads(monkeypatch):
    from xagent.web.services.task_worker_capacity import TaskWorkerCapacity

    monkeypatch.setenv("XAGENT_TASK_WORKER_MAX_CONCURRENT_TASKS", "2")
    capacity = TaskWorkerCapacity()
    barrier = Barrier(8)

    def reserve(task_id):
        barrier.wait(timeout=5)
        return capacity.reserve(task_id)
